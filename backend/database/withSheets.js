...
async function getSpreadSheet(auth) {
  ...
  try {
    const response = await getSheet({
      spreadsheetId: '14H2onKYrcLHxksWFiXRn0njYkhCE8i9lYNVOhpH5Oxw',
      ranges: ['Sheet1', 'Sheet2']
    })
    if (response) {
      const [videos, courses] = response.data.valueRanges.map(vr => vr.values.slice(1))
      seedData(videos, courses)
    }
  } catch (error) {
